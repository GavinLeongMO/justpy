...
class EventHandler {
    ...
    handle(props, event, form_data, aux) {
        ...
        if (!this.event_sender.websocket_ready && this.event_sender.use_websockets) {
        ...
        let e = {
            ...
            'page_id': this.event_sender.page_id,
            'websocket_id': this.event_sender.websocket_id
        };
        ...
        if ((event instanceof Event) && (event.target.type == 'file')) {
            ...
            this.files_chosen[event.target.id] = event.target.files;
            var files = [];
        ...
        if (modifiers && modifiers.debounce) {
            ...
            modifiers.debounce.timeout = setTimeout(function () {
                    ...
                    if (!callNow) this.send_to_server(set_e, 'event', props.jp_props.debug);
                }
            ...
            if (!modifiers.throttle.timeout) {
                ...
                modifiers.throttle.timeout = setTimeout(function () {
                        this.send_to_server(set_e, 'event', props.jp_props.debug);
            ...
            this.send_to_server(e, 'event', props.jp_props.debug);
        }
 ...
 */
    send_to_server(e, event_type, debug_flag=false) {
        if (debug_flag) {
            console.log('Sending message to server:');
            console.log({'type': event_type, 'event_data': e});
        }
        if (this.event_sender.use_websockets) {
            if (this.event_sender.web_socket_closed) {
                if (debug_flag) {
            ...
            if (this.event_sender.websocket_ready) {
                this.event_sender.socket.send(data);
            } else {
                setTimeout(function () {
                    this.event_sender.socket.send(data);
                }, 1000);
